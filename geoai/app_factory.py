...
def _load_config(config_path: Optional[str] = None) -> dict:
    if config_path is None:
        config_path = os.environ.get("GEOAI_CONFIG", "configs/config.yaml")
    p = Path(config_path)
    if not p.exists():
...
def build_metadata_db(cfg: dict):
    ...
    db_path = Path(cfg.get("storage", {}).get("db_path", "data/geoai_metadata.db"))
...
def build_quality_assessor(cfg: dict):
    ...
    if name == "basic":
        from geoai.providers.quality.basic import BasicQualityAssessor
